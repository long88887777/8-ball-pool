...
export function bounceHanBlend(v: Vector3, w: Vector3) {
  ...
  const baseFactor = isCheckSide ? Math.cos(Math.atan2(v.y, v.x)) : 1
  const sideStrength = Math.min(1, Math.abs(w.z) / 20)
  const factor = isCheckSide
    ? baseFactor * (1 - sideStrength * 0.4)
    : baseFactor
